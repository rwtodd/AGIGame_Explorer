...
This document maps Sierra's 16-color SCI graphics onto the Flutter pipeline already used for AGI (`PictureSlice`, `PictureSlicer`, `PlayfieldPainter`, `ViewTextureAtlas`). Pics, views, fonts, cursors, `DisplayProfile`, the parameterized slicer, `PlayfieldActorSprite`, window overlays, and the SCI PMachine VM are implemented (Stages 1–9). Roadmap: [sci0_dual_engine_architecture.md](sci0_dual_engine_architecture.md) §8.
